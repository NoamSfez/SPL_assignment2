...
public abstract class MicroService implements Runnable { 
	...
	private String name;
	private HashMapSafe<Class<? extends Message>, Callback> callbacks;
	private MessageBusImpl m;
    ...
    public MicroService(String name) {
    	this.name = name;
    	callbacks= new 	HashMapSafe<Class<? extends Message>, Callback>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
    	System.out.println(this.name + " subscribeEvent");
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
    	System.out.println(this.name + " subscribeBroadcast");
    	callbacks.put(type, callback);
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) throws InterruptedException {
    	System.out.println(this.name + " sendEvent " + e.hashCode());
    	Future<T> f = m.sendEvent(e);
    	futures.add(f);
    	return f;
    }
    ...
    protected final void sendBroadcast(Broadcast b) {
    	System.out.println(this.name + " sendBroadcast");
    	m.sendBroadcast(b);
    }
    ...
    protected final <T> void complete(Event<T> e, T result) {
    	System.out.println(this.name + " complete " + e.hashCode());
    	m.complete(e, result);
    }
    ...
    public final void run() {
		...
		while(!Thread.currentThread().isInterrupted()) {
			try {
				...
				callbacks.get(mes.getClass()).call(mes);
			} catch (InterruptedException e) {
//				System.out.println("run Microservice 2");
				Thread.currentThread().interrupt();
			}
    	}
    	m.unregister(this);
